...
def print_accuracy_attr(accuracy_list_per_type,accuracy_list_per_attr, label, epoch=0, batch_iter=0):
    if label == "Test":
        f = open('Test_Acc_Attr_Per_Type.txt', 'a')
        f2 = open('Test_Acc_Per_Attr.txt', 'a')
        for k in range(accuracy_list_per_type.size(0)):
            for i in range(accuracy_list_per_type.size(1)):
                f.write("%f  " % (accuracy_list_per_type[k,i]))
            f.write('\n')
        f.close()
        for k in range(0, accuracy_list_per_type.size(0)):
             for i in range(accuracy_list_per_attr.size(1)):
                f2.write("%f  " % (accuracy_list_per_attr[k,i]))
             f2.write('\n')
        f2.close()
        logging.info("[ %s Accu ] of Test Dataset:" % (label))
    else:
    ...
    if label=="Train":
        f = open('Train_Acc_Attr_Per_Type.txt', 'a')
        for i in range(len(accuracy_list_per_type)):
           f.write("%f  " % (accuracy_list_per_type[i]))
        f.write('\n')
        f.close()


    if label == "Validate":
        f = open('Val_Acc_Attr_Per_Type.txt', 'a')
        f2 = open('Val_Acc_Per_Attr.txt', 'a')
        for k in range(accuracy_list_per_type.size(0)):
            for i in range(accuracy_list_per_type.size(1)):
                f.write("%f  " % (accuracy_list_per_type[k,i]))
            f.write('\n')
