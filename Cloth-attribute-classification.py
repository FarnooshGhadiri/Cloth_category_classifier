    ...
    for i, data in enumerate(data_loader):
        ...
        try:
           output_binary, output_softmax, bin_loss, cls_loss = forward_batch(model, criterion_softmax, criterion_binary, inputs, target_softmax,target_binary, opt, "Validate")
            ...
            print('failed to forward the batches')
        ...
        try:
           ...
           logging.info("batch accuracu %f" % (batch_accuracy_bin[0]))
        except:
            print('fail to calculate the accuracy')
          # accumulate accuracy
...
def calc_accuracy(output_binary, output_softmax, target_softmax,target_binary, score_thres, top_k,opt):
    ...
    Sum_all_corr_attr = torch.zeros(len(top_k), target_binary.size(1))
    Sum_all_gt_attr = torch.zeros(len(top_k), target_binary.size(1))
    ...
    for k in top_k:
        ...
        Correct_pred_bin = Topk_binary.float() * target_binary.cpu()
        ...
        Target_minus_corrected = target_binary.cpu() * (1 - Topk_binary)
        Num_GT_attr = target_binary * attr_type.repeat(batch_size, 1)
        for b in range(batch_size):
            if target_binary.cpu()[b, :].sum() > k:
                ...
                tmp_sample_GT = torch.zeros(target_binary.cpu.size(1))
                tmp_sample_GT[torch.stack(New_sample_GT)] = 1
                Num_GT_attr[b, :] = Num_GT_attr[b, :] * tmp_sample_GT
                target_binary.cpu()[b, :] = target_binary.cpu()[b, :] * tmp_sample_GT

        target_binary = target_binary.cpu() + Correct_pred_bin
        Num_GT_attr_per_class[k_num, 0] = target_binary.cpu().sum()
        Num_GT_attr_per_class[k_num, 1] = (Num_GT_attr == 1).sum() + int(correct_attr_type[k_num, 1])
        Num_GT_attr_per_class[k_num, 2] = (Num_GT_attr == 2).sum() + int(correct_attr_type[k_num, 2])
        Num_GT_attr_per_class[k_num, 3] = (Num_GT_attr == 3).sum() + int(correct_attr_type[k_num, 3])
        Num_GT_attr_per_class[k_num, 4] = (Num_GT_attr == 4).sum() + int(correct_attr_type[k_num, 4])
        Num_GT_attr_per_class[k_num, 5] = (Num_GT_attr == 5).sum() + int(correct_attr_type[k_num, 5])
        Sum_all_corr_attr[k_num] = Correct_pred_bin.sum(0)
        Sum_all_gt_attr[k_num] = target_binary.cpu().sum(0)
        k_num += 1


    return acc_soft, correct_attr_type, Num_GT_attr_per_class,  Sum_all_corr_attr, Sum_all_corr_attr
